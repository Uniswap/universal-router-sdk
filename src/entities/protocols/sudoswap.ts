  ...
  encode(planner: RoutePlanner, config: TradeConfig): void {
    for (const order of this.orders) {
      const calldata = SudoswapTrade.INTERFACE.encodeFunctionData('robustSwapETHForSpecificNFTs', [
        order.swaps.map((swap) => {return { swapInfo: swap.swapInfo, maxCost: swap.maxCost }}),
        order.ethRecipient,
        order.nftRecipient,
        order.deadline,
      ])
      const value = order.swaps.reduce((prevVal, swap) => {return prevVal.add(swap.maxCost)}, BigNumber.from(0))
