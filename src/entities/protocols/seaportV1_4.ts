import abi from '../../../abis/SeaportV1_4.json'
import { BigNumber } from 'ethers'
import { Interface } from '@ethersproject/abi'
import { BuyItem, Market, NFTTrade, TokenType } from '../NFTTrade'
import { TradeConfig } from '../Command'
import { RoutePlanner, CommandType } from '../../utils/routerCommands'
import { ConsiderationItem, FulfillmentComponent, Order, SeaportData } from './seaport'
...
export class SeaportV1_4Trade extends NFTTrade<SeaportData> {
  ...
  encode(planner: RoutePlanner, config: TradeConfig): void {
    for (const order of this.orders) {
      ...
      if (advancedOrders.length == 1) {
        calldata = SeaportV1_4Trade.INTERFACE.encodeFunctionData('fulfillAdvancedOrder', [
          advancedOrders[0],
          [],
          SeaportV1_4Trade.OPENSEA_CONDUIT_KEY,
          order.recipient,
        ])
      } else {
        calldata = SeaportV1_4Trade.INTERFACE.encodeFunctionData('fulfillAvailableAdvancedOrders', [
          advancedOrders,
          [],
          orderFulfillments,
      ...
      planner.addCommand(CommandType.SEAPORT_V1_4, [this.getTotalPrice().toString(), calldata], config.allowRevert)
    }
  }
